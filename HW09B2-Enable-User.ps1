<#
 # Program:     Enables AD User account(s)
 # File:        HW09B2-Enable-Users.ps1
 # Author:      Anand Ramesh Kannan
 # Description: Enables Active Directory User(s) account(s).
 #  
 # Date:        2017 Apr 15
 #                - Created
 #              2017 Apr 18
...
<#
.SYNOPSIS
Enables Active Directory (AD) user(s) account(s).
.DESCRIPTION
Enables all the specified Active Directory (AD) user(s) account(s).
...
The output of this script is a PSCustom Object with the password specific required properties.
.PARAMETER pServer
...
.EXAMPLE
(HW09A1-Get-User.ps1 -pServer $sv -pCredential $cr -pFilter { Name -Like "HW09*" }).SAMAccountName | HW09B2-Enable-User.ps1 -pConfirm:$False -pServer $sv -pCredential $cr
Enables all AD Users whose account names start with “HW09”
.EXAMPLE
HW09B2-Enable-User.ps1 -pServer $sv -pCredential $cr -pFilter { Name -Like "HW09*" }
Enables all AD Users whose SAMAccountNames begin with HW09.
 #>
    [CmdletBinding(
...
PROCESS {
    ...
    "One" {
        ...
        foreach ($user in $pAccountName) {
          ...
          $username | `
           Enable-ADAccount -Server $pServer -Credential $pCredential  -Confirm:$pConfirm -ErrorAction Stop
           ...
           if($username -eq $null) {
            ...
            Write-Warning "The password for the user does not meet the complexity requirements of the domain."
            Write-Error "Account could not be activated."
      ...
      "Two" {
            ...
            if($pFilter.ToLower().Contains($allDelete2) -or $pFilter.CompareTo($allDelete1) -eq 0) {
                Write-Warning "The filter: '$($pFilter)' could potentially Enable all users. We have disabled it for security purposes."
                exit
            }#if
            try {
            ...
            Write-Warning "The password for the user does not meet the complexity requirements of the domain."
            Write-Error "Unable to activate the account(s)."
            Write-Warning "If you were trying to perform operations on accounts other than those in OU=HW09OU_RameAn, you do not have sufficient access permissions."
            Write-Error "Access Denied."
            }
            break
